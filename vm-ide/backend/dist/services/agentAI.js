...
class AgentAI {
     ...
     */
    async run(prompt, contextInfo, executeTool, emit, isContinuation = false) {
        this.aborted = false;
        if (!isContinuation) {
            const userMessage = contextInfo
                ? `${prompt}\n\n--- Context ---\n${(0, agentTools_1.redactSecrets)(contextInfo)}`
                : prompt;
     ...
     */
    async planOnly(prompt, contextInfo) {
        const userMessage = contextInfo
            ? `${prompt}\n\n--- Context ---\n${(0, agentTools_1.redactSecrets)(contextInfo)}`
            : prompt;
...
- Do not output environment variables or secrets.`,
    ...
    // ─── Private ───────────────────────────────────────────────────
    async converse() {
        return this.callWithRetry(() => this.client.messages.create({
            model: this.modelId,
            max_tokens: 4096,
            temperature: 0.2,
    ...
    async callWithRetry(fn) {
        for (let i = 0; i < 3; i++) {
            try {
                ...
                const isRateLimit = err instanceof sdk_1.default.RateLimitError ||
                    (err instanceof sdk_1.default.APIError && err.status === 429);
                if (isRateLimit && i < 2) {
...
exports.AgentAI = AgentAI;
//# sourceMappingURL=agentAI.js.map