...
const agentAI_1 = require("./agentAI");
...
const agentAIs = new Map();
const agentToolInstances = new Map();
// ─── Orchestrator ────────────────────────────────────────────────
class AgentOrchestrator {
    /**
     * Create a new agent session and start processing.
     */
    async startSession(sshSessionId, prompt, context, options, emit) {
        // Verify SSH session
        const sshSession = (0, sessionStore_1.getSession)(sshSessionId);
        if (!sshSession) {
        ...
        for (const s of agentSessions.values()) {
            if (s.state === "running" || s.state === "planning")
                activeSessions++;
        ...
        const tools = new agentTools_1.AgentTools(sshSessionId, context.workspaceRoot);
        const ai = new agentAI_1.AgentAI();
    ...
    resumeSession(sessionId) {
        ...
        if (session.state === "awaiting_permission")
            throw new Error("Cannot resume. Session is awaiting user permission.");
     ...
     */
    grantPermission(sessionId) {
        const session = agentSessions.get(sessionId);
        if (!session)
            throw new Error("Agent session not found");
        if (session.state !== "awaiting_permission")
            throw new Error("Session is not awaiting permission");
    ...
    async rollbackSession(sessionId) {
        ...
        if (!tools)
            throw new Error("Agent tools not found");
     ...
     */
    getSessionHistory(sshSessionId) {
        const history = [];
     ...
     */
    deleteSession(sessionId, sshSessionId) {
        const session = agentSessions.get(sessionId);
     ...
     */
    async planOnly(sshSessionId, prompt, context) {
        const contextInfo = await this.buildContextInfo(sshSessionId, context);
        const ai = new agentAI_1.AgentAI();
     ...
     */
    async continueSession(sessionId, prompt, context, emit) {
        const session = agentSessions.get(sessionId);
        if (!session)
            throw new Error("Agent session not found");
        ...
        const ai = agentAIs.get(sessionId);
        if (!ai)
            throw new Error("Agent AI instance not found");
    ...
    // ─── Private ───────────────────────────────────────────────────
    async runAgentLoop(sessionId, contextInfo, emit, prompt, isContinuation = false) {
        const session = agentSessions.get(sessionId);
        if (!session)
            return;
        const tools = agentToolInstances.get(sessionId);
        ...
        const executeTool = async (toolName, args) => {
            ...
            while (session.state === "paused") {
                ...
                if (session.state !== "paused")
                    break; // state changed externally (stopped, etc.)
            }
            if (session.state === "stopped")
                throw new Error("Session stopped");
            // Check if this tool requires explicit permission
            const permCheck = (0, agentTools_1.checkToolPermission)(toolName, args, session.context.workspaceRoot, session.options.autoRunCommands ?? true);
            if (permCheck.requiresPermission) {
                ...
                if (session.state === "stopped")
                    throw new Error("Session stopped while awaiting permission");
     ...
     */
    async buildContextInfo(sshSessionId, context) {
        const parts = [];
        const sshSession = (0, sessionStore_1.getSession)(sshSessionId);
        if (!sshSession)
            return "";
        ...
        if (context.currentFile) {
            try {
                const stats = await (0, types_1.sftpStat)(sshSession.sftp, context.currentFile);
                if (stats.size < 100_000) {
                    const buf = await (0, types_1.sftpReadFile)(sshSession.sftp, context.currentFile);
        ...
        const execQuiet = (cmd) => sshExecutor_1.sshExecutor.exec({ sessionId: sshSessionId, command: cmd, timeout: 8_000 })
