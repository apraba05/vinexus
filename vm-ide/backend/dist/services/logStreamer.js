...
exports.logStreamer = exports.LogStreamer = void 0;
...
class LogStreamer {
     ...
     */
    async fetchLogs(sessionId, service, lines = 50) {
     ...
     */
    async startStreaming(sessionId, service, onEntry, onError) {
        let stopped = false;
        // Start journalctl -f in streaming mode
        const execPromise = sshExecutor_1.sshExecutor.execStreaming({
            sessionId,
     ...
     */
    parsePlainLogs(output, defaultUnit) {
        const entries = [];
        for (const line of output.split("\n")) {
            if (!line.trim())
                continue;
     ...
     */
    parseSingleLine(line, defaultUnit) {
        if (!line.trim())
            return null;
        // Try ISO format first (from -o short-iso)
        const isoMatch = line.match(/^(\d{4}-\d{2}-\d{2}T[\d:]+[+-]\d{4})\s+\S+\s+(\S+?)(?:\[\d+\])?:\s*(.*)$/);
        if (isoMatch) {
            return {
...
exports.logStreamer = new LogStreamer();
//# sourceMappingURL=logStreamer.js.map