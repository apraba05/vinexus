...
function getFromEmail() {
  return process.env.RESEND_FROM_EMAIL || "no-reply@vinexus.space";
}
...
export async function sendPasswordResetEmail(email: string, token: string) {
  ...
  await getResend().emails.send({
    from: getFromEmail(),
    to: email,
    subject: "Reset your Vinexus password",
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 480px; margin: 0 auto; padding: 40px 20px;">
        ...
        <p style="color: #a1a1aa; line-height: 1.6; margin-bottom: 24px;">
          You requested a password reset for your Vinexus account. Click the button below to set a new password. This link expires in 1 hour.
        </p>
...
export async function sendVerificationEmail(email: string, token: string) {
  ...
  await getResend().emails.send({
    from: getFromEmail(),
    to: email,
    subject: "Verify your Vinexus email",
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 480px; margin: 0 auto; padding: 40px 20px;">
        <h2 style="color: #fafafa; margin-bottom: 16px;">Verify your email</h2>
        <p style="color: #a1a1aa; line-height: 1.6; margin-bottom: 24px;">
          Welcome to Vinexus! Please verify your email address by clicking the button below. This link expires in 24 hours.
        </p>
